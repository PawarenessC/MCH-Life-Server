...
ABOUT;
    public function onEnable() {
        ...
        $this->Setting = new Config($this->getDataFolder() . "Setting.yml", Config::YAML, array("コンフィグバージョン(編集禁止)" => self::CONFIG_VERSION, "参加時にプレイヤーの情報を表示する(true or false)" => "false", "UBan時にプレイヤーの情報を表示する(true or false)" => "false"));
    ...
    bool {
        ...
        if ($command->getName() === "uban") {
            ...
            if ($this->getSetting("UBan時にプレイヤーの情報を表示する(true or false)") == "false") {
...
§b------ §6$name §b------
INFO_3;
                    $array = ["ban = 1, warn = 1" => "info_1", "ban = 1, warn = 0" => "info_2", "ban = 0, warn = 1" => "info_3"];
                    $info = $array[$type];
                    $sender->sendMessage($ {
                    $info
                    });
...
UBANBYTEXT;
    ...
    public function setDefaultStatus(Player $player) {
        $name = $player->getName();
        if ($this->isWarned($name) == 0) return;
